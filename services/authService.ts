...
const createUserDocument = async (
  ...
  while (!docSaved && saveAttempts < maxAttempts) {
    try {
      ...
      console.error(
        `[Auth] Document save attempt ${saveAttempts} failed:`,
        error.message,
      );

      if (saveAttempts >= maxAttempts) {
        throw new Error(
          `Failed to save user data after ${maxAttempts} attempts: ${error.message}`,
        );
      }
  ...
  try {
    ...
    if (!verifySnap.exists()) {
      console.error(
        "[Auth] Document verification failed - not found after save",
      );
    }
...
export const signUpWithEmail = async (userData: {
  ...
  try {
    ...
    const emailPromise = sendEmailVerification(firebaseUser).catch((err) =>
  ...
  const fetchRole = async (
    ...
    try {
      ...
      if (error.message?.includes("Target ID already exists")) {
        if (attempt < 2) {
          console.warn(
            "[Auth] Firestore internal target collision detected, retrying getUserRole...",
          );
      ...
      if (
        ...
        if (attempt < 2) {
          console.warn(
            "[Auth] Firestore SDK internal error detected, retrying getUserRole...",
          );
      ...
      if (
        ...
        if (attempt < 2) {
          console.warn(
            "[Auth] Transient Firestore error detected, retrying getUserRole...",
          );
...
export const signInWithEmail = async (credentials: {
  ...
  try {
    ...
    try {
      ...
      if (
        roleError.message?.includes("INTERNAL ASSERTION FAILED") ||
        roleError.message?.includes("Unexpected state")
      ) {
        console.warn(
          "[Auth] Firestore SDK error during login, treating as offline",
        );
...
export const deleteAccount = async (): Promise<{
  ...
  try {
    ...
    return {
      success: false,
      error: error?.message || error?.code || "Failed to delete account",
    };
  }
};
...
export const logOut = async (): Promise<{
  ...
  try {
    ...
    // Clear supplier dashboard cache (if any)
    if (user?.uid) {
      try {
        const { clearCachedSupplierDashboardData } =
          await import("@/services/supplierAuthService");
        await clearCachedSupplierDashboardData(user.uid);
      } catch {
        // ignore
      }
    }
...
export const getUserDataWithCache = async (
  uid: string,
