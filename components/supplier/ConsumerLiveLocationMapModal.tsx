...
export function ConsumerLiveLocationMapModal({
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToConversation(
      ...
      (conversation) => {
        ...
        setResolvedName(
          conversation.consumerName || consumerNameProp || "Customer",
        );
  ...
  const ageMs = updatedAt ? nowMs - updatedAt.getTime() : undefined;

  const status = (() => {
    if (!location || !updatedAt || ageMs === undefined) {
      return { label: "Waiting for customer to share location", color: "#666" };
    }
    if (!isActivelySharing) {
      return {
        label: "Last known location (customer offline or not sharing)",
        color: "#B26A00",
      };
    }
    if (ageMs <= 60000) return { label: "Live now", color: "#0A7D00" };
    if (ageMs <= 5 * 60000)
      return { label: "Updated recently", color: "#B26A00" };
        ...
        <View style={styles.footer}>
          {location ? (
            ...
            <Text style={styles.hint}>
              Ask the customer to tap “Share Live Location” after placing their
              order. Last shared locations stay saved even when they go offline.
            </Text>
          )}
        </View>
      </SafeAreaView>
