...
const normalizeLiveLocation = (
  raw: any,
): Conversation["consumerLiveLocation"] | undefined => {
...
export const mapConversationDoc = (
  ...
  consumerLiveLocationUpdatedAt: toDateOrUndefined(
    data.consumerLiveLocationUpdatedAt,
  ),
  ...
  unreadCount:
    data.supplierUnreadCount ??
    data.consumerUnreadCount ??
    data.unreadCount ??
    0,
...
export const markMessagesAsRead = async (
  ...
  try {
    ...
    if (conversationData) {
      ...
      const roleUnreadField = isSupplier
        ? "supplierUnreadCount"
        : "consumerUnreadCount";
...
const getConsumerPhoneNumber = async (
  consumerId: string,
  ...
  try {
    ...
    console.warn(
      "[Chat] Could not resolve consumer phone number:",
      error?.message || error,
    );
  }
...
// Cache for consumer phone numbers to avoid repeated Firestore reads

const consumerPhoneCache = new Map<string, string | undefined>();
...
export const subscribeToSupplierConversations = (
  ...
  return onSnapshot(
    ...
    (querySnapshot: any) => {
      ...
      const phoneFetchPromises: Promise<void>[] = [];
      ...
      for (const docSnap of querySnapshot.docs) {
        ...
        // Check cache for consumer phone number

        const consumerId = data.consumerId;

        let consumerPhone = data.consumerPhone as string | undefined;

        if (!consumerPhone && consumerId) {
          if (consumerPhoneCache.has(consumerId)) {
            consumerPhone = consumerPhoneCache.get(consumerId);
          } else {
            // Fetch phone number asynchronously

            const fetchPromise = getConsumerPhoneNumber(consumerId).then(
              (phone) => {
                consumerPhoneCache.set(consumerId, phone);
              },
            );

            phoneFetchPromises.push(fetchPromise);
          }
        }
      ...
      // Fire callback immediately with current data

      callback(Array.from(byId.values()));

      // Resolve phone numbers in background and update if needed

      if (phoneFetchPromises.length > 0) {
        Promise.all(phoneFetchPromises).then(() => {
          // Rebuild conversations with resolved phone numbers
          ...
          for (const docSnap of querySnapshot.docs) {
            ...
            if (!consumerPhone && data.consumerId) {
              consumerPhone = consumerPhoneCache.get(data.consumerId);
            }
          ...
          callback(Array.from(updatedById.values()));
        });
      }
    },
...
export const updateConsumerLiveLocation = async (
  ...
  try {
    ...
    if (!existing.exists()) {
      return {
        success: false,
        error: "Order conversation not found. Please place the order again.",
      };
    }
    ...
    console.error(
      "[Chat] Update consumer live location error:",
      error?.code,
      error?.message,
    );
...
export const subscribeToConversation = (
  ...
  return onSnapshot(
    ...
    (error: any) => {
      console.error(
        "[Chat] subscribeToConversation error:",
        error?.code,
        error?.message,
      );
