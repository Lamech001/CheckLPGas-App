...
export default function VerifyEmailScreen() {
  ...
  const { email, role, from, verifying: verifyingParam, oobCode } = useLocalSearchParams<{ 
    email: string; 
    role: string; 
    from: string;
    verifying: string;
  ...
  }, [oobCode, verifyingParam]);

  const handleDeepLinkVerification = async (code: string) => {
    setIsVerifying(true);
  ...
  const handleOpenEmail = async () => {
    ...
    try {
      ...
      if (canOpen) {
        ...
        Alert.alert('Email App', 'Please check your email app for the verification link.');
      }
    } catch {
      Alert.alert('Email App', 'Please check your email app for the verification link.');
    }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.instructions}>
          Click the link in the email to verify your account and access your {role || 'supplier'} dashboard.
        </Text>

        {/* Email Button */}
        <TouchableOpacity style={styles.emailButton} onPress={handleOpenEmail}>
          <FontAwesome5 name="envelope-open" size={18} color="#fff" style={styles.buttonIcon} />
